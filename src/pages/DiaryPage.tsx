...
export function DiaryPage() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true)
    try {
      setNotes(await listNotes({ archived: false, kind: 'diary', limit: query ? undefined : 100 }))
    } finally {
      setLoading(false)
    }
  }, [query])
  ...
  async function toggleArchive(note: Note) {
    await updateNote(note.id, { is_archived: !note.is_archived, kind: 'diary', diary_date: note.diary_date })
    await refresh()
  }
  ...
  return (
    <section className="mx-auto max-w-3xl">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-semibold text-ink">日记</h1>
          <p className="mt-2 text-muted">按日期保存，每天一篇，也可以补写过去的日子。</p>
        </div>
        <Link className="link-button" to="/write">写日记</Link>
      </div>

      <div className="mb-5 grid gap-3 sm:grid-cols-[1fr_auto]">
        <label className="relative block">
          <Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted" size={16} />
          <input className="field pl-9" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索日记" />
        </label>
      ...
      {!loading && visible.length ? (
        <div className="rounded-md border border-line bg-surface/70 px-5">
          {visible.map((note) => (
            <NoteListItem key={note.id} note={note} onPin={() => undefined} onArchive={toggleArchive} onDelete={remove} />
          ))}
        </div>
      ) : null}
      {!loading && !visible.length ? <EmptyState title="还没有日记。" action={<Link className="link-button" to="/write">从今天开始写一点</Link>} /> : null}
    </section>
  )
}
