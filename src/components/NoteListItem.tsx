...
import { readableDiaryDate, relativeTime } from '../lib/date'
      ...
      <div className="flex items-start justify-between gap-4">
        <Link to={`/notes/${note.id}`} className="min-w-0 flex-1">
          <div className="mb-2 flex flex-wrap items-center gap-2 text-sm text-muted">
            {isDiary && note.diary_date ? (
              <time dateTime={note.diary_date}>日记 · {readableDiaryDate(note.diary_date)}</time>
            ) : (
              <time>{relativeTime(note.updated_at)}</time>
            )}
            {!isDiary && note.is_pinned ? <span className="rounded-md bg-accent/10 px-2 py-0.5 text-accent">置顶</span> : null}
            {note.tags.map((tag) => (
